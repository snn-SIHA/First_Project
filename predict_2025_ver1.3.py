# predict_2025
# ver 1.3
# 업데이트 내용 : 최고 기온 및 최저 기온을 학습 과정에 반영
...
ytrain = df1[['평균기온(℃)','평균최저기온(℃)','평균최고기온(℃)','전년대비']]
ytest = df2[['평균기온(℃)','평균최저기온(℃)','평균최고기온(℃)','전년대비']]

# 스케일링
STS = StandardScaler()
scaled = STS.fit_transform(ytrain[['평균기온(℃)','평균최저기온(℃)','평균최고기온(℃)']])
ytrain.loc[:, '평균기온(℃)'] = scaled[:,0]
ytrain.loc[:, '평균최저기온(℃)'] = scaled[:,1]
ytrain.loc[:, '평균최고기온(℃)'] = scaled[:,2]
scaled = STS.transform(ytest[['평균기온(℃)','평균최저기온(℃)','평균최고기온(℃)']])
ytest.loc[:, '평균기온(℃)'] = scaled[:,0]
ytest.loc[:, '평균최저기온(℃)'] = scaled[:,1]
ytest.loc[:, '평균최고기온(℃)'] = scaled[:,2]
...
#--------------------------------------------------
# 결과 데이터프레임화
dfp = pd.DataFrame(STS.inverse_transform(pre[:,:3]).round(1)) # 예측값
dfp = dfp.iloc[:,0]
print(f'>>> TRACE\n{dfp}')
df2['예측기온(℃)'] = dfp
...
plt.xlabel('')
plt.ylim(-3.5,32.5)
plt.title(f'2025년 {locate_file} 기온 예측 (예측모델 ver.1.3)')
plt.legend(loc='best')
plt.grid(axis='y',linestyle='--',alpha=0.35)
plt.tight_layout()
plt.savefig(f'C:/Mtest/project_first/pic/{locate_folder}_2025_L_1.3.png')
plt.show()
...
plt.xlabel('')
plt.ylim(-1.5,1.5)
plt.title(f'2025년 {locate_file} 전년 대비 온도 변화 (예측모델 ver.1.3)')
plt.grid(axis='y',linestyle='--',alpha=0.35)
plt.tight_layout()
plt.savefig(f'C:/Mtest/project_first/pic{locate_folder}_2025_B_1.3.png')
plt.show()
